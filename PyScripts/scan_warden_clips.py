"""Articulation scan (recipe 7.5) for the Hollow Warden's timed attacks.
Tracks the sword-hand bone's world Z and reach so the real cut frame can be
windowed out of the long knight routines.
"""
import unreal

DEST = "/Game/Art/WardenSkelV1"
CLIPS = ["Slash", "Judgment", "Charged", "Voidcast"]
...
for clip in CLIPS:
    ...
    print(f"WDSCAN {clip}: len={length:.2f}s hand={hand} hips={hips}")
    t = 0.0
    ...
    while t <= length:
        ...
        t += step
    print(f"WDSCAN {clip}_PEAKS: handZ peak={peak_z:.1f} @frac {peak_z_frac:.3f} | "
