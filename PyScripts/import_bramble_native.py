"""THE BRAMBLEHULK enters the engine — mesh + 8 clips + material + slam scans."""
import os
import unreal
...
MESH_FBX = os.path.join(DROPS, "rig", "bramble_rig_rigged_character_fbx_url.fbx")
CLIPS = {
    "Dormant": "anim_dormant\\bramble_dormant_animation_fbx_url.fbx",
    "Alert": "anim_alert\\bramble_alert_animation_fbx_url.fbx",
    "Walk": "anim_walk\\bramble_walk_animation_fbx_url.fbx",
    "Slam": "anim_slam\\bramble_slam_animation_fbx_url.fbx",
    "Quake": "anim_quake\\bramble_quake_animation_fbx_url.fbx",
    "Roar": "anim_roar\\bramble_roar_animation_fbx_url.fbx",
    "HitReact": "anim_hitreact\\bramble_hitreact_animation_fbx_url.fbx",
    "Soothed": "anim_soothed\\bramble_soothed_animation_fbx_url.fbx",
}
PROBE = "/Game/_ProbeBramble"
DEST = "/Game/Art/BrambleSkelV1"
NAME = "SCB2Bramble"
TARGET_HALF_EXTENT_Z = 88.0   # 2.2 m Colossus x the family 0.8 = 176 uu
...
print(f"BRM_PROBE: native z={z:.1f} -> scale {scale:.4f}")
EAL.delete_directory(PROBE)

if EAL.does_directory_exist(DEST):
    ok = EAL.delete_directory(DEST)
    if not ok:
        raise SystemExit("BRM_WIPE_FAILED (CDO lock?) — bump folder version")
...
print(f"BRM_MESH: extent z={mesh.get_bounds().box_extent.z:.1f} (target {TARGET_HALF_EXTENT_Z})")

bound = 0
for clip, rel in CLIPS.items():
    import_skeletal(os.path.join(DROPS, rel), DEST, f"A_Bramble_{clip}", scale, skel=skeleton)
    ...
    if isinstance(a, unreal.AnimSequence):
        ...
        bound += int(ok)
        print(f"BRM {clip}: len={a.get_play_length():.2f}s {'BOUND' if ok else 'ORPHANED'}")
    else:
        print(f"BRM {clip}: NO_ANIMSEQUENCE")

base_tex = unreal.load_asset(f"{DEST}/texture_0")
mat = tools.create_asset("M_BramblePBR", DEST, unreal.Material, unreal.MaterialFactoryNew())
...
print(f"BRM_MAT_ASSIGNED: {len(mats)} slot(s)")

# ---- slam scans: hand height tells when stone meets ground ----
opts = unreal.AnimPoseEvaluationOptions()
opts.evaluation_type = unreal.AnimDataEvalType.RAW
for clip in ("Slam", "Quake"):
    a = unreal.load_asset(f"{DEST}/A_Bramble_{clip}_Anim")
    ...
    while t <= length:
        ...
        for hand in (lhand, rhand):
            if hand:
                ...
                row.append(f"hz={p.z:6.1f}")
        print(f"BRMSCAN {clip}: frac={t/length:.3f}  {'  '.join(row)}")
        t += step

EAL.save_directory(DEST, recursive=True)
print(f"BRM_{'SUCCESS' if bound == len(CLIPS) else 'PARTIAL'}: {bound}/{len(CLIPS)} bound, scale={scale:.4f}")
