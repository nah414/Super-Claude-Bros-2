"""THE GLADE PROWLER enters the engine in his OWN body — mesh + 10 clips + material.
V4 native pipeline into virgin /Game/Art/ProwlerSkelV1 (stage-31 drops; the Warlord
loan is repaid). De-glow law applies; the visor's glow is the MoodLight's job.
"""
import os
import unreal
...
DROPS = r"C:\Users\Atomn\mario2\_prep\meshy_api_drops\prowler"
MESH_FBX = os.path.join(DROPS, "rig", "prowler_rig_rigged_character_fbx_url.fbx")
CLIPS = {
    "Idle": "anim_idle\\prowler_idle_animation_fbx_url.fbx",
    "Walk": "anim_walk\\prowler_walk_animation_fbx_url.fbx",
    "Swing": "anim_swing\\prowler_swing_animation_fbx_url.fbx",
    "Chop": "anim_chop\\prowler_chop_animation_fbx_url.fbx",
    "Sweep": "anim_sweep\\prowler_sweep_animation_fbx_url.fbx",
    "HitReact": "anim_hitreact\\prowler_hitreact_animation_fbx_url.fbx",
    "Stagger": "anim_stagger\\prowler_stagger_animation_fbx_url.fbx",
    "Defeat": "anim_defeat\\prowler_defeat_animation_fbx_url.fbx",
    "Taunt": "anim_taunt\\prowler_taunt_animation_fbx_url.fbx",
}
PROBE = "/Game/_ProbeProwler"
DEST = "/Game/Art/ProwlerSkelV1"
NAME = "SCB2Prowler"
TARGET_HALF_EXTENT_Z = 76.0   # 1.9 m rig x the family 0.8 = 1.52 m = 152 uu
...
print(f"PROWL_PROBE: native z={z:.1f} -> scale {scale:.4f}")
EAL.delete_directory(PROBE)

if EAL.does_directory_exist(DEST):
    ok = EAL.delete_directory(DEST)
    if not ok:
        raise SystemExit("PROWL_WIPE_FAILED (CDO lock?) — bump folder version")
...
print(f"PROWL_MESH: extent z={mesh.get_bounds().box_extent.z:.1f} (target {TARGET_HALF_EXTENT_Z})")

bound = 0
for clip, rel in CLIPS.items():
    src = os.path.join(DROPS, rel)
    if not os.path.isfile(src):
        print(f"PROWL {clip}: DROP_MISSING")
        continue
    import_skeletal(src, DEST, f"A_Prowler_{clip}", scale, skel=skeleton)
    a = unreal.load_asset(f"{DEST}/A_Prowler_{clip}_Anim")
    if isinstance(a, unreal.AnimSequence):
        ok = a.get_editor_property("skeleton") == skeleton
        bound += int(ok)
        print(f"PROWL {clip}: len={a.get_play_length():.2f}s {'BOUND' if ok else 'ORPHANED'}")
    else:
        print(f"PROWL {clip}: NO_ANIMSEQUENCE")

base_tex = unreal.load_asset(f"{DEST}/texture_0")
mat = tools.create_asset("M_ProwlerPBR", DEST, unreal.Material, unreal.MaterialFactoryNew())
...
metal.set_editor_property("r", 0.2)   # mossed chrome, dulled by the glade's damp
ok_metal = MEL.connect_material_property(metal, "", unreal.MaterialProperty.MP_METALLIC)
MEL.recompile_material(mat)
EAL.save_loaded_asset(mat)
print(f"PROWL_MAT: base={ok_base} rough={ok_rough} metal={ok_metal}")
...
print(f"PROWL_MAT_ASSIGNED: {len(mats)} slot(s)")

EAL.save_directory(DEST, recursive=True)
print(f"PROWL_{'SUCCESS' if bound == len(CLIPS) else 'PARTIAL'}: {bound}/{len(CLIPS)} bound, scale={scale:.4f}")
