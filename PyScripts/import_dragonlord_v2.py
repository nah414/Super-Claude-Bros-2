"""THE LUMEN DRAGONLORD — RE-RIG V2 import (Phase 2, Path A). Identical pipeline to
import_dragonlord_native.py, but reads the dragonlord_v2 cook (clean 80k-quad rig,
emerald/brass PBR) and lands in a VIRGIN /Game/Art/DragonSkelV2 so DragonSkelV1 stays
intact for instant rollback. Same NAME/clip slot names/target height, so the C++ only
needs its load paths repointed V1 -> V2. The Unlight inherits this rig for free.
"""
import os
import unreal
...
DROPS = r"C:\Users\Atomn\mario2\_prep\meshy_api_drops\dragonlord_v2"
...
PROBE = "/Game/_ProbeDragonV2"
DEST = "/Game/Art/DragonSkelV2"
NAME = "SCB2Dragon"
TARGET_HALF_EXTENT_Z = 120.0   # match V1 exactly so the C++ scale/offsets carry over
...
assert os.path.isfile(MESH_FBX), f"DRG_V2_NO_RIG: {MESH_FBX} (cook not finished?)"
...
print(f"DRG_V2_{'SUCCESS' if bound == len(CLIPS) else 'PARTIAL'}: {bound}/{len(CLIPS)} bound, scale={scale:.4f}")
