"""THE VOID STALKER enters the engine — mesh + 9 clips + material, one session.
V4 native pipeline into virgin /Game/Art/StalkerSkelV1. De-glow law applies.
"""
import os
import unreal
...
DROPS = r"C:\Users\Atomn\mario2\_prep\meshy_api_drops\stalker"
MESH_FBX = os.path.join(DROPS, "rig", "stalker_rig_rigged_character_fbx_url.fbx")
CLIPS = {
    "Idle": "anim_idle\\stalker_idle_animation_fbx_url.fbx",
    "Run": "anim_run\\stalker_run_animation_fbx_url.fbx",
    "Strike": "anim_strike\\stalker_strike_animation_fbx_url.fbx",
    "Combo": "anim_combo\\stalker_combo_animation_fbx_url.fbx",
    "Lunge": "anim_lunge\\stalker_lunge_animation_fbx_url.fbx",
    "Stagger": "anim_stagger\\stalker_stagger_animation_fbx_url.fbx",
    "HitReact": "anim_hitreact\\stalker_hitreact_animation_fbx_url.fbx",
    "Defeat": "anim_defeat\\stalker_defeat_animation_fbx_url.fbx",
    "Taunt": "anim_taunt\\stalker_taunt_animation_fbx_url.fbx",
}
PROBE = "/Game/_ProbeStalker"
DEST = "/Game/Art/StalkerSkelV1"
NAME = "SCB2Stalker"
TARGET_HALF_EXTENT_Z = 69.0   # 1.72 m x the family 0.8 = 137.6 uu
...
print(f"STK_PROBE: native z={z:.1f} -> scale {scale:.4f}")
EAL.delete_directory(PROBE)

if EAL.does_directory_exist(DEST):
    ok = EAL.delete_directory(DEST)
    if not ok:
        raise SystemExit("STK_WIPE_FAILED (CDO lock?) — bump folder version")
...
print(f"STK_MESH: extent z={mesh.get_bounds().box_extent.z:.1f} (target {TARGET_HALF_EXTENT_Z})")

bound = 0
for clip, rel in CLIPS.items():
    import_skeletal(os.path.join(DROPS, rel), DEST, f"A_Stalker_{clip}", scale, skel=skeleton)
    a = unreal.load_asset(f"{DEST}/A_Stalker_{clip}_Anim")
    if isinstance(a, unreal.AnimSequence):
        ok = a.get_editor_property("skeleton") == skeleton
        bound += int(ok)
        print(f"STK {clip}: len={a.get_play_length():.2f}s {'BOUND' if ok else 'ORPHANED'}")
    else:
        print(f"STK {clip}: NO_ANIMSEQUENCE")

base_tex = unreal.load_asset(f"{DEST}/texture_0")
mat = tools.create_asset("M_StalkerPBR", DEST, unreal.Material, unreal.MaterialFactoryNew())
...
print(f"STK_MAT_ASSIGNED: {len(mats)} slot(s)")

EAL.save_directory(DEST, recursive=True)
print(f"STK_{'SUCCESS' if bound == len(CLIPS) else 'PARTIAL'}: {bound}/{len(CLIPS)} bound, scale={scale:.4f}")
